...
export function LeadBadge({ level }: { level?: string }) {
  ...
  const label = normalized.includes("warm") || normalized === "m" ? "Warm" : normalized.includes("cold") || normalized === "l" ? "Cold" : "Hot";
  const color =
    label === "Hot"
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 rounded-md px-2.5 py-1 text-xs font-bold", color)}>
      {label === "Hot" && <Flame className="h-3.5 w-3.5" />}
      {label}
    </span>
  );
}
